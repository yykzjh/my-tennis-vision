...
class PlayerDetector(object):
    ...
    def __init__(self, dtype, model_name, quantize, fp):
        self.dtype = dtype
        ...
        self.mot_tracker = utils.Sort(max_age=10, min_hits=3, iou_threshold=0.05)
        ...
        if model_name == "faster_rcnn":
            if quantize:
                ...
                img_size = (640, 640)
                # 初始化TRT类
                self.trt_util = utils.TRTUtil(img_size=img_size)
                # 初始化onnx模型
                self.ort_session = ort.InferenceSession(r"./pretrain/faster_rcnn.onnx", providers=['CUDAExecutionProvider'])
            else:
            ...
            self.PERSON_LABEL = 0
            self.PERSON_SCORE_MIN = 0.65
            # 初始化模型参数
            img_size = (640, 640)
            yolov8_onnx_path = os.path.join(r"./pretrain/yolov8_onnx_static", model_name + ".onnx")
            yolov8_trt_path = os.path.join(r"./pretrain/yolov8_trt_static", model_name + "_" + fp + ".trt")
            # 初始化TRT类
            self.trt_util = utils.TRTUtil(yolov8_onnx_path, yolov8_trt_path, fp, img_size)

            if quantize:
                if fp == "onnx":
                    # 初始化onnx模型
                    self.ort_session = ort.InferenceSession(yolov8_onnx_path, providers=['CUDAExecutionProvider'])
                else:
                    # 初始化模型参数
                    img_size = (640, 640)
                    yolov8_onnx_path = os.path.join(r"./pretrain/yolov8_onnx_static", model_name + ".onnx")
                    ...
                    # 初始化TRT类
                    self.trt_util = utils.TRTUtil(yolov8_onnx_path, yolov8_trt_path, fp, img_size)
                    self.trt_util.init_model()
            else:
                # 初始化yolov8模型
                yolov8_pt_path = os.path.join(r"./pretrain/yolov8_pt", model_name + ".pt")
                self.yolo_model = YOLO(yolov8_pt_path)
    ...
    def _detect(self, image, person_min_score=None):
        ...
        if self.model_name == "faster_rcnn":
            if self.quantize:
                input_image, ratio = self.trt_util.pre_process(image)
                input_name = self.ort_session.get_inputs()[0].name
                boxes_name = self.ort_session.get_outputs()[0].name
                labels_name = self.ort_session.get_outputs()[1].name
                scores_name = self.ort_session.get_outputs()[2].name
                result = self.ort_session.run([boxes_name, labels_name, scores_name], {input_name: input_image})
                p = self.trt_util.faster_rcnn_post_process(result, image, ratio)
            else:
            ...
            if self.quantize:
                if self.fp == "onnx":
                    ...
                    input_image, ratio = self.trt_util.pre_process(image_np)
                    input_name = self.ort_session.get_inputs()[0].name
                    output_name = self.ort_session.get_outputs()[0].name
                    result = self.ort_session.run([output_name], {input_name: input_image})
                    p = self.trt_util.post_process(result, 0.25, 0.65, image_np, ratio)
                else:
                    image_np = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
                    input_image, ratio = self.trt_util.pre_process(image_np)
                    engine_infer_output = self.trt_util.inference(input_image)
                    p = self.trt_util.post_process(engine_infer_output, 0.25, 0.65, image_np, ratio)
            else:
                result = self.yolo_model.predict(image, save=False, verbose=False)
                p = self.trt_util.yolov8_pytorch_post_process(result)
...
if __name__ == "__main__":
    ...
    detection_model = PlayerDetector(dtype=dtype)

    video = cv2.VideoCapture('VideoInput/video_input1.mp4')
    print('Video FPS ', video.get(cv2.CAP_PROP_FPS))
    # get videos properties
    fps, length, v_width, v_height = utils.get_video_properties(video)
    ...
    # props
    fps, length, width, height = utils.get_video_properties(cap)
