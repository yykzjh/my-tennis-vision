...
from lib.utils.CourtReference import CourtReference
...
class CourtDetector(object):
    ...
    def detect(self, frame, verbose=0):
        ...
        t2 = time.time()
        print("二值化阈值分割用时为：{:.8f}".format((t2 - t1) * 1000))
        ...
        t2 = time.time()
        print("细化线宽用时为：{:.8f}".format((t2 - t1) * 1000))
        ...
        t2 = time.time()
        print("霍夫变换检测水平和垂直的直线用时为：{:.8f}".format((t2 - t1) * 1000))
        ...
        t2 = time.time()
        print("获取实际球场线和参照球场线之间透视变换的变换矩阵用时为：{:.8f}".format((t2 - t1) * 1000))
        ...
        t2 = time.time()
        print("将参照球场变换后返回用时为：{:.8f}".format((t2 - t1) * 1000))
        return lines
    ...
    def _detect_lines(self, gray):
        ...
        t2 = time.time()
        print("霍夫变换提取直线用时为：{:.8f}".format((t2 - t1) * 1000))
        if self.verbose:
        ...
        t2 = time.time()
        print("划分水平和垂直的直线用时为：{:.8f}".format((t2 - t1) * 1000))
        if self.verbose:
        ...
        t2 = time.time()
        print("合并直线用时为：{:.8f}".format((t2 - t1) * 1000))
        if self.verbose:
            display_lines_on_frame(self.frame.copy(), horizontal, vertical)
    ...
    def _find_homography(self, horizontal_lines, vertical_lines):
        ...
        print("确定变换矩阵和分数平均用时为：{:.8f}".format(time_sum / time_cnt), time_cnt)
