...
class CourtDataset(Dataset):
    ...
    def __init__(self, opt, mode="train"):
        ...
        self.opt = opt
        self.mode = mode
        self.root_dir = self.opt["dataset_path"]
    ...
    def __getitem__(self, index):
        ...
        label = self._load_label(label_path, image)
        # 构造数据增强的关键点格式
        keypoints = label[:, 1:]
    ...
    @staticmethod
    def _load_label(label_path, img):
        # 读取标签文件
        ...
        for lbs in labels:
            ...
            # 存储关键点信息
            kps.append([int(lbs[0]), lb[0] * img.shape[1], lb[1] * img.shape[0]])
        return np.array(kps)
