...
params = {
    ...
    "dataset_name": "Court",  # 数据集名称

    "dataset_path": r"./datasets/court",  # 数据集路径
    ...
    "model_name": "ResNet50_Revise",  # 模型名称，可选["ResNet50_Revise", "MobileNetV2"]

    "in_channels": 3,  # 模型最开始输入的通道数,即模态数

    "classes": 19,  # 模型最后输出的通道数,即类别总数

    "resume": None,  # 是否重启之前某个训练节点，继续训练;如果需要则指定.state文件路径

    "pretrain": r"./pretrain/keypoints_model.pth",  # 是否需要加载预训练权重，如果需要则指定预训练权重文件路径
